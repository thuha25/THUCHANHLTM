...
def import_examinators(csv_path):
    ...
    cur.execute(
        'CREATE TABLE IF NOT EXISTS examinator(id TEXT PRIMARY KEY, name TEXT, date_of_birth TEXT, work_unit TEXT)')
...
def import_rooms(csv_path):
    ...
    cur.execute(
        'CREATE TABLE IF NOT EXISTS exam_room(id TEXT PRIMARY KEY, description TEXT)')
    with open(csv_path, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
...
if __name__ == '__main__':
    ...
    with open('CanBoCoiThi.csv', 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            try:
